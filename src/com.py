...
VGTT_JOB = "https://snake:8080/view/Asp/view/ASP2%20Dashboard/job/03_OV_VGTT_Tuner_AT_Cmds/71/CONFIG_HW=FC6100,label=VGTT/artifact/results/"
...
def timing(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        func(*args, **kwargs)
        stop = datetime.now()
        #logger.info('Time execution %s, %s : %s sec' % \
...
def decompressed_tgz(tgz_file, output_directory):
    ...
    with tarfile.open(tgz_file) as tgz_file:
        ...
        if tgz_info:
            ...
            #logger.info("%s untarred" % tgz_file)
        else:
